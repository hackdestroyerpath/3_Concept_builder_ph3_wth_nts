...
5. Если протокол имеет status `available`, открыть только его файл и прямые зависимости.
6. Если status `deferred` или `planned`, не имитировать выполнение. Создать/обновить issue/backlog и перейти к допустимому implementation step.
...
| `execution/concept_export` | [execution_protocols/concept_export_protocol.md](execution_protocols/concept_export_protocol.md) | `execution` | read-only export precheck, closed export, WIP export, package local-open и export blocker diagnosis | precheck result без mutation либо persisted package/readback либо точный blocker |
...
| Нужен export precheck, WIP/closed export или диагностика package blockers | `execution` | `execution/concept_export` | `available` | optional selected concept для precheck; selected concept и readable verified state/revision для mutation; local page/issue/dependency registries; required output refs; export request | read-only precheck result либо manifest/page map/compact issue summary/archive/validation report, либо exact blocker | `closed_concept_exported`; `wip_exported_not_final`; `export_precheck_complete`; `blocked` |
| Issue готов к закрытию или package готовится к commit/export | `service/execution` | `common/final_validation` | `available` | state, requirements/contract/output if any, page registry, issue registry, export evidence if applicable | validation report, issue/contract/export coverage | pass/pass_with_deferred_items/blocked |
...
Для `execution/concept_export` human и machine catalog используют одну operational картину:

- `export_precheck` является read-only и может вернуть `blocked_no_concept` без selected concept; он не создаёт package, registry/state или persistence mutation;
- package mutation требует selected concept, readable `state_revision`, `integrity_status = verified`, parseable local registries и explicit requested export type;
- closed route блокируют identity/entrypoint, integrity/revision, readiness/output, registry/source-link/orphan, blocking issue/dependency не normalized `ready`, validation/language, package consistency/local-open, collision и persistence failures;
- WIP route всегда `not_final`; переносимые defects требуют exact limitations, `draft_with_notice` и user acceptance, а missing package entrypoint или root escape блокируют оба типа;
- completion signal: precheck result без mutation, persisted/read-back closed/WIP package либо exact blocker с next step.
...
| Issue tombstones | [Issues/_tombstones/README.md](../Issues/_tombstones/README.md) | перед tombstone cleanup и удалением неисторических files |
